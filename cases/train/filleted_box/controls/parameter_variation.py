...
length, width, height, radius = (20.0, 16.0, 12.0, 2.0)
...
while explorer.More():
    ...
    if abs(first.Y()) < 1e-7 and abs(first.Z()) < 1e-7 and abs(last.Y()) < 1e-7 and abs(last.Z()) < 1e-7 and abs(last.X() - first.X()) > length / 2.0:
