...
box = BRepPrimAPI_MakeBox(24.0, 14.0, 10.0).Shape()
...
while explorer.More():
    ...
    if abs(first.Y()) < 1e-7 and abs(first.Z()) < 1e-7 and abs(last.Y()) < 1e-7 and abs(last.Z()) < 1e-7 and abs(last.X() - first.X()) > 12.0:
