"""Build only the six preregistered M26 candidates offline."""
...
try:  # Supports both package import and direct script execution.
    from tools.audit_sequence_paired_multi_inner_loop_pocket import EXPANSION, ROOT, build_shape, canonical_sequence
    ...
    from audit_sequence_paired_multi_inner_loop_pocket import EXPANSION, ROOT, build_shape, canonical_sequence
...
def _stable(entry: dict[str, Any]) -> None:
    with tempfile.TemporaryDirectory(prefix="brep2code-m26-a-") as first, tempfile.TemporaryDirectory(prefix="brep2code-m26-b-") as second:
...
def build(expansion_path: Path = EXPANSION, output_root: Path = ROOT) -> list[str]:
    ...
    if expansion.get("selection_status") != "preregistered_before_candidate_production" or not isinstance(rows, list) or len(rows) != 6:
        raise ValueError("M26 production requires exactly six preregistered rows")
    produced = []
    for entry in rows:
