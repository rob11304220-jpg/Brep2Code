...
def test_catalog_and_manifests_validate() -> None:
    ...
    assert {
        (manifest.split, item.case.case_id) for manifest in manifests for item in manifest.cases
    } == EXPECTED_CASES
...
def test_catalog_binds_cases_to_mechanism_registry_and_dossiers() -> None:
    ...
    assert {item.metadata["mechanism"] for manifest in manifests for item in manifest.cases} <= set(
...
def test_case_rejects_hash_mismatch(tmp_path: Path) -> None:
    ...
    (case_root / "case.json").write_text(json.dumps(_metadata(sha256="0" * 64)), encoding="utf-8")
...
@pytest.mark.parametrize(
    ...
    [
        (
            "smoke/box/input.step",
            "c79a4f205ed7bf698a367832f855c716abfee1c9891848384d0e46ad026db927",
        ),
        (
        ...
        (
            "eval/cylinder/input.step",
            "98f7bbdd9e3f24e3532f4adfe0837f5a0862e6f614bfb998f1520a297d607f4c",
        ),
        (
            "eval/box_held_out/input.step",
            "d41f288a26a789eb4beb18094fdcae7282d20592e40d791c0fa62e86fa49195b",
        ),
        (
            "eval/cylinder_held_out/input.step",
            "7855afb6e59f82460facff646da83122fc71122dc16cf8e7e0a2d861855a72e0",
        ),
        (
            "eval/through_cut_held_out/input.step",
            "0abef779812f5f55b610bb4ad40766a43b671a3b86328dc079bdf55429a1f62f",
        ),
        (
            "eval/blind_cut_held_out/input.step",
            "b30007ffc7d0f16fac496a4c5bc2e3ef02d90ada698cea1a8a8cc25c42146e3e",
        ),
        (
            "train/filleted_box/input.step",
            "e6d8f0ffd097cd24010165a0bcd030d89e5dce9cc5b43a07cb2eb8401f48fd1e",
        ),
        (
            "eval/filleted_box_held_out/input.step",
            "d3d2b9dcf1f9d1ff6aa688dd09a06c0a40df9bb5a5cac6176dab405fe4ee7bd9",
        ),
    ],
)
